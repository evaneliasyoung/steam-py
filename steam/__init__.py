...
class user:
   def __init__(self, steam64):
      ...
      if(len(steam64) != 17): raise Exception("The Steam64ID provided is invalid")
   ...
   def getName(self):
      ...
      return parElem.find('bdi').getText()
   def getStatus(self):
      ...
      status['main'] = mainElem.getText().replace('Currently ', '').lower()
      if(status['main'] == "offline"):
         status['last'] = descElem.getText().replace('Last Online ', '').lower()
      elif(status['main'] == "in-game"):
         status['game'] = descElem.getText()
      return status
   def getLevel(self):
   ...
   def getCounts(self):
      ...
      for i in range(0, len(conEls)):
         key = lblEls[i].getText().strip().lower()
         if(key != 'inventory'): counts[key] = int(conEls[i].getText().strip())
      return counts
   def getBadges(self):
...
class Badge:
   ...
   def getLevXP(self):
      ...
      for i in range(0, len(lvlxp)):
         lvlxp[i] = MakeInt(lvlxp[i])
      lvlxp.reverse()
      lvlxp.append(None)
      return lvlxp
   def getDate(self):
